...
#endif
#if DT_INST_NODE_HAS_PROP(0, mdm_sim_select_gpios)
static const struct gpio_dt_spec sim_select0_gpio = GPIO_DT_SPEC_INST_GET_BY_IDX(0, mdm_sim_select_gpios, 0);
static const struct gpio_dt_spec sim_select1_gpio = GPIO_DT_SPEC_INST_GET_BY_IDX(0, mdm_sim_select_gpios, 1);
#endif
...
#if DT_INST_NODE_HAS_PROP(0, mdm_sim_select_gpios)
	LOG_INF("Default to removable SIM");
	gpio_pin_set_raw(sim_select0_gpio.port, sim_select0_gpio.pin, 0);
    gpio_pin_set_raw(sim_select1_gpio.port, sim_select1_gpio.pin, 0);
	k_sleep(K_MSEC(250));
#endif
...
#if DT_INST_NODE_HAS_PROP(0, mdm_sim_select_gpios)
	ret = gpio_pin_configure_dt(&sim_select0_gpio, GPIO_OUTPUT_LOW);
	if (ret < 0) {
		LOG_ERR("Failed to configure %s pin", "sim_selec0");
		goto error;
	}

    ret = gpio_pin_configure_dt(&sim_select1_gpio, GPIO_OUTPUT_LOW);
	if (ret < 0) {
		LOG_ERR("Failed to configure %s pin", "sim_select1");
		goto error;
	}
#endif
...
NET_DEVICE_DT_INST_OFFLOAD_DEFINE(0, modem_init, NULL,
				  &mdata, NULL,
				  CONFIG_MODEM_QUECTEL_BG96_INIT_PRIORITY,
				  &api_funcs, MDM_MAX_DATA_LENGTH);

/* Register NET sockets. */
NET_SOCKET_OFFLOAD_REGISTER(quectel_bg96, CONFIG_NET_SOCKETS_OFFLOAD_PRIORITY,
